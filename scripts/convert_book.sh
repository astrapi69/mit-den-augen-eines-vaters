...
# Define variables
BOOK_NAME="ai_for_everyone_book"
...
cat \
    ...
    "$INPUT_DIR/front-matter/introduction.md" \
...
# Convert to PDF
pandoc --verbose "$OUTPUT_DIR/$BOOK_NAME.md" --metadata-file=$METADATA -o "$OUTPUT_DIR/$BOOK_NAME.pdf" --css=$STYLE

# Convert to EPUB
pandoc --verbose "$OUTPUT_DIR/$BOOK_NAME.md" --metadata-file=$METADATA -o "$OUTPUT_DIR/$BOOK_NAME.epub"
